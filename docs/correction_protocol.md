...
생성 HWPX의 내부 XML은 한글에서 저장한 네이티브 HWPX 본문 스타일을 기준으로 맞춥니다. A4 수치, `pagePr`, 본문 `borderFill`, 기본 글꼴 테이블, 20pt 함초롬바탕 참조 ID가 한글 생성본과 다르면 같은 dialog 값이어도 화면 줄배치가 달라질 수 있습니다. 다만 루프 중간 검토본은 하네스가 줄 위치를 추정해서 만들기 때문에 한글 줄 나눔은 반드시 글자 단위(`breakNonLatinWord="BREAK_WORD"`)로 둡니다. 단어 단위 줄 나눔은 양쪽 정렬에서 단어 사이 공백을 과하게 벌릴 수 있습니다.
