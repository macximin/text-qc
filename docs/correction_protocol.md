...
생성 HWPX의 내부 XML은 한글에서 저장한 네이티브 HWPX 본문 스타일을 기준으로 맞춥니다. A4 수치, `pagePr`, 본문 `borderFill`, 기본 글꼴 테이블, 20pt 함초롬바탕 참조 ID가 한글 생성본과 다르면 같은 dialog 값이어도 화면 줄배치가 달라질 수 있습니다.
