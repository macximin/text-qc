...
- `editorial-pass`: 적극 편집자 모드. 전역 정합성 3-pass와 화별 수동 딥다이브, 정합성 리포트 이후 replace/delete/insert_before/insert_after 기반 윤문, 중복 삭제, 브리지 추가를 수행. 회차별 공백 포함 4000자 기준을 정본 선택의 강한 원칙으로 보며, HWP/HWPX 대신 text 후보본과 Markdown diff를 사용.
