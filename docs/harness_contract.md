...
편집자 모드는 HWP/HWPX를 기본 작업물로 쓰지 않습니다. `apply-changes-text`가 `final_manuscript/editorial_candidate.txt`와 `corrections/editorial_diff.md`를 생성합니다. 다만 루프 중간 확인을 위해 `render-marked-manuscript-hwpx`로 `human-facing/*_marked_manuscript.hwpx`를 만들 수 있습니다. 이 HWPX는 원문 순서 그대로 `ⓐ{원문|수정}`과 `ⓐⓐ(의견: ... / 제안: ...)`을 삽입한 검토본이며 원고 적용본이 아닙니다. `ⓐⓐ`는 모두 의견 메모로만 표시하고 원문을 치환하지 않습니다. HWPX 파란줄 납품본은 별도 납품 요구가 있을 때만 사용합니다.
...
3. `corrections/changes.json`을 검증하고 `render-marked-manuscript-hwpx`로 원문형 기호 적용 HWPX를 생성한다.
...
`validate-report --run-root`는 기본 1장 보고서인 `human-facing/one_page_report.md`를 검증합니다. 최종 개선 보고서를 납품 대상으로 쓸 때는 파일을 직접 지정합니다.
...
1. 전역 정합성 3-pass를 수행하고 `llm-facing/adversarial_audit_3pass.md`에 확정/강등/유보를 남긴다.
2. `llm-facing/episode_deep_dive.md`에 회차별 수동 독해를 남긴다. 자동 evidence만 복사하지 않고 앞뒤 회차와 장면 맥락을 직접 확인한다.
3. `llm-facing/consistency_report.md`에 편집자 모드로 넘길 항목과 보류 항목을 분리한다.
4. 그 뒤에만 `corrections/changes.json`에 적극 편집 후보를 작성한다.
...
기호 보존이 기본값입니다. `ⓚ`, 대괄호 UI, 회차 제목, 특수기호는 삭제하지 않고, 송고 위생 문제인지 작중 장치인지 먼저 판정합니다.
