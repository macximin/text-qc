...
runs/{run_id}/
  ...
  editorial_pass/
    qc/
      qc_ssot_v0.jsonl
      qc_ledger_EPxxx-yyy.jsonl
      diff_policy_v0.jsonl
      qc_review.html
  final_manuscript/
    README.md
...
- `review/chapter_length_flags.jsonl`: 공백 포함 4000자 미만 회차 후보.
...
- `submission/manual_review_queue.jsonl`: profile별 작업 큐. `required_for_gate=true`인 행만 해당 run의 필수 작업입니다.
- `submission/manual_review_submission.json`: 사람이 완료한 감리 결과. 완료 상태는 `gate_profile`의 required axes/workflow를 기준으로 검증합니다.
...
HWP/HWPX 또는 인간 검수 경유 원고는 아래 사전 리스크 체크리스트를 갖고 시작합니다. 이 항목들은 확정 사실이 아니라 초기 감리 가설이며, 직접 근거 확인 전에는 적용 가능한 수정으로 승격하지 않습니다.

- AI 작성/AI-slop 가능성: 반복 리캡, 장면 접합, 시간 역류, 정보 상태 회귀, 균질한 문장 리듬을 먼저 의심합니다.
- glossary 미정렬 가능성: 고유명사, 실명/가명, 약칭/이니셜은 전역 치환하지 않고 정본 보류로 시작합니다.
- 정합성/중복 문제 가능성: 중복 회차, 중복 리캡, 사건 상태 회귀, 앞뒤 화 브리지 결손을 root gate로 봅니다.
- 인간 검수자 내부 메모 잔존 가능성: 대괄호, 괄호 대안문, 슬래시 병기, 비독자-facing 코멘트를 내부 메모 후보로 분류합니다.
- 화수 표기/분할 이상 가능성: HWP 계열 추출에서는 회차 번호 누락, 중복 번호, 검수자 삭제로 인한 gap, oversized merged episode를 내용 검수 전 먼저 닫습니다.

## Global Context Scan First

HWP 계열, AI 작성 의심, glossary 미정렬, 중복/화수 이상 가능성이 있는 원고는 회차별 수정 전에 전역 컨텍스트 스캔을 먼저 수행합니다.

전역 스캔은 수정 지시가 아니라 지도입니다. 앞쪽이 틀렸는지 뒤쪽이 틀렸는지, 실제 역사 기준으로 맞추는 것이 비용 폭탄인지, 작중 세계관으로 고정하는 것이 더 안전한지 판단하기 위해 전체 분포를 먼저 봅니다.
...
전역 스캔에서 회차 순서/분할 문제가 확인되면 본문 교정 전에 source integrity repair candidate를 먼저 만듭니다. 이 단계는 최종 원고 수정이 아니며, 원 run의 `source.txt`를 덮어쓰지 않습니다.

```powershell
.\scripts\novel-qc-loop.ps1 plan-source-repair --run-root "workspace\...\runs\..." --version v1
```

95% 이상 확신 가능한 source-level 수정은 직접 반영할 수 있습니다. 이때도 원 추출본은 보존하고, working source/final manuscript에만 반영합니다.

```powershell
.\scripts\novel-qc-loop.ps1 plan-source-repair --run-root "workspace\...\runs\..." --version v1 --accept-direct --confidence-threshold 95
```

산출물은 `RUN/source_integrity/v1/` 아래에 둡니다.

- `source_repaired_v1.txt`: 문서 순서와 안전 분할만 반영한 후보 원문입니다.
- `source_accepted_v1.txt`: `--accept-direct` 실행 시 95% 이상 직접 반영 기준을 통과한 working source입니다.
- `episodes_repaired_v1/`: 후보 원문을 다시 회차별로 나눈 확인용 텍스트입니다.
- `source_repair_changes_v1.jsonl`: 변경 원장입니다. 문서 재정렬, 붙은 회차 꼬리표 분리, 차단 항목을 한 줄 한 판단으로 남깁니다.
- `chapter_sequence_v1.jsonl`: 후보 원문의 회차 순서 장부입니다.
- `chapter_metrics_v1.jsonl`: 후보 원문 기준 회차 길이 지표입니다.
- `source_repair_diff_v1.md`: 원본 회차 순서와 후보 회차 순서의 lightweight diff입니다. 원문 전문 diff가 아니라 sequence diff입니다.
- `source_repair_manifest_v1.json`: 입력, 출력, 정책, 경로를 고정한 manifest입니다.

자동 적용 가능한 것은 두 가지뿐입니다.

- 파일명에서 명확히 추론되는 문서 묶음 순서 재정렬
- `이전 화 마지막 문장.54화`처럼 직전 회차보다 1 큰 회차 꼬리표가 문장 끝에 붙은 경우의 분리

직접 반영된 줄은 `source_repair_changes_v1.jsonl`에서 `status=accepted_direct`, `confidence_percent >= 95`로 표시합니다.

아래는 차단으로 남기고 원본/HWP 확인 후 처리합니다.

- 누락 회차 범위
- 중복 회차 variant
- `/98화(99)화/ 끝`처럼 후보 번호가 병기된 애매한 회차 꼬리표

## LLM-facing

`llm-facing/`은 내부 작업자와 AI 에이전트가 읽는 폴더입니다. 길어도 됩니다.

## Authority/Gate Profile

하네스 권위는 아래 순서로 정렬합니다.

1. 코드 권위: `novel_qc_loop.protocol`의 상수와 `GATE_PROFILE_DEFINITIONS`.
2. Run 권위: `run_manifest.json`의 `gate_profile`과 artifact 경로.
3. 제출 권위: `evidence/submission/manual_review_submission.json`.
4. 납품 권위: `validate-report`를 통과한 `human-facing` 보고서와 최종 후보본.

Gate profile은 작업 범위를 계층화합니다.

- `proofread`: 오탈자, 띄어쓰기, 문장부호, 송고 위생만 닫습니다.
- `correction`: `ⓐ`/`ⓐⓐ` 교정안과 판단용 마커 검수본을 닫습니다.
- `editorial`: 적극 편집 후보를 위한 최소 정합성 장부와 편집 근거를 닫습니다.
- `consistency`: primary/blind/total/adversarial 정합성 감리를 닫되, 외부 납품 보고서는 별도 gate로 둡니다.
- `delivery`: 기존 full gate입니다. 모든 정합성 감리와 human-facing 보고서 검증을 요구합니다.

`manual_review_queue.jsonl`에는 profile별 필수 여부가 `required_for_gate`로 표시됩니다. `delivery`가 아닌 profile에서는 full 납품 gate를 임의로 끌어오지 않습니다.

`delivery` profile에서 반드시 남길 것:
...
납품 대상으로 쓰는 최종 보고서는 `validate-report`를 통과해야 합니다. `delivery` profile의 `--run-root` 기준 검증은 한국어 비율, 내부 작업어 노출, placeholder, 주장-근거 쌍뿐 아니라 `manual_review_submission.json`이 감리 완료/검증 상태인지도 확인합니다.
...
## Lightweight QC Ledger

가벼운 순차 검수에서는 HWPX/MD 검수본을 원장으로 쓰지 않습니다. `editorial_pass/qc/*.jsonl`이 원장이고, HTML/MD/diff는 필요할 때 생성하는 보기용 산출물입니다.

- `qc_ssot_v0.jsonl`: 확정 오류, 보류, 장부 재구성, 정책 선택 등 실행 권위가 있는 최소 SSOT입니다.
- `qc_ledger_EPxxx-yyy.jsonl`: 10화 단위 검수 원장입니다. 한 줄이 한 판단입니다.
- `diff_policy_v0.jsonl`: diff 생성 규칙입니다. diff 자체는 원장이 아닙니다.
- `qc_review.html`: 필요할 때만 `render-qc-html`로 생성하는 사람용 보기 파일입니다.

기본 검증:

```powershell
.\scripts\novel-qc-loop.ps1 validate-qc-jsonl --run-root "workspace\...\runs\..." --include-ssot
```

필요할 때만 HTML 보기 생성:

```powershell
.\scripts\novel-qc-loop.ps1 render-qc-html --run-root "workspace\...\runs\..." --include-ssot
```

`confirmed_error`와 `confirmed_internal_marker`만 실행 후보가 될 수 있습니다. `canon_hold`, `ledger_needed`, `policy_choice`, `blocked`, `needs_human`은 적용하지 않고 원장에 남깁니다.
...
만족 기준은 P0/P1 정합성 이슈가 해결 또는 작가 판단 보류로 분리되고, 중복/정본 선택과 회차별 공백 포함 4000자 기준이 닫히며, 재평가에서 새 P0/P1 회귀가 없는 상태입니다.
...
편집자 모드는 전역 감리 직후 바로 실행하지 않습니다. `gate_profile`에 맞는 `consistency_first_editorial_gate`를 통과한 뒤 실행합니다. `delivery`/`consistency` profile은 아래 full gate를 요구하고, `editorial` profile은 `manual_review_queue.jsonl`의 `required_for_gate=true` 항목과 `consistency_report.md`의 진입 판정을 우선합니다.
...
회차별 공백 포함 글자수는 4000자 이상을 강한 원칙으로 둡니다. `inspection.json`, `facts/chapter_metrics.jsonl`, `review/chapter_length_flags.jsonl`, `submission_gate.json`은 이 기준과 미달 회차를 드러냅니다. 4000자 미만 회차는 결락, 중복, 분할 오류, 정본 선택 보류 후보로 먼저 검토합니다.

중복 회차는 아래 기준으로 처리합니다.

- 완전 동일 중복: 뒤쪽 또는 파일명 범위에서 벗어난 블록을 `ⓐ(삭제)` 후보로 올릴 수 있다.
- 비동일 중복: 먼저 정본 선택을 `ⓐⓐ` 판단으로 남긴다.
- 정본 후보는 공백 포함 4000자 이상이어야 한다. 삭제 후 남는 정본이 4000자 미만이면 삭제 확정 대신 결락/추가/정본 선택 보류로 둔다.
...
최종 승인 패키지는 TXT 원고와 human-facing HTML 보고서를 기본 산출물로 만듭니다. 보고서는 결론을 먼저 두고, 산출물/상태 지표/처리 내역/정책 판정/검증/근거 파일 순서로 이어집니다. 최종 납품 보고서 기본 밀도는 `closing_full`이며, 전체 누적 최종 마감 보고서로 렌더링합니다.

```powershell
.\scripts\novel-qc-loop.ps1 render-final-delivery --run-root "workspace\...\runs\..." --version v1
.\scripts\novel-qc-loop.ps1 render-final-delivery --run-root "workspace\...\runs\..." --version v1 --report-density closing_full
```

기본 출력은 `final_delivery/v1_final_approved_package/`이며, `delivery_manifest.json`에 source/final/delivery TXT 해시, 회차 수, 최신 scan 상태, QC 검증 결과를 기록합니다. HWPX/PDF는 기본 납품물이 아니라 별도 요청이 있을 때만 추가 export로 생성합니다.

최종 보고서 밀도는 아래처럼 씁니다.

- `brief`: 내부 확인용 요약.
- `standard`: 통상 납품용 상태 보고서.
- `closing_full`: 최종 마감용 누적 보고서. 기본값이며, 정책 판정, glossary SSOT, AI-slop 표면, hold/watchlist 분리, 검증/재봉인 상태를 모두 포함합니다.

Hold는 출고 차단과 watchlist를 분리합니다.

- `blocking_hold`: 출고 전 반드시 해결해야 하는 P0/P1 또는 차단 항목.
- `policy_watchlist`: 실명/가명, 정본 선택, 소설적 허용처럼 추적은 필요하지만 출고 차단이 아닌 항목.
- `style_watchlist`: 문체, 톤, 리듬, 표현 취향처럼 원문의 맛 보존과 충돌할 수 있는 항목.

AI-slop 표면은 별도 스캐너로 확인합니다. 회차 끝 메타 표식, 괄호형 구명/주석, 영어 뒤 괄호 번역, A사/XX년 placeholder, 내부 메모, 도구명 잔재를 후보로 잡습니다.

```powershell
.\scripts\novel-qc-loop.ps1 scan-ai-slop --input "workspace\...\runs\...\final_manuscript\final_manuscript.txt" --output "workspace\...\runs\...\consistency_integrity\ai_slop_scan.jsonl"
```

Glossary SSOT가 있으면 `render-final-delivery`는 최신 `glossary_current*.jsonl` 또는 manifest의 `glossary_current_path`를 읽어 최종 보고서에 정본/별칭/금지 표면/잔여 카운트를 요약합니다. 최종 원고가 재수정되면 이전 패키지 해시와 현재 TXT 해시를 비교해 stale 여부를 manifest와 보고서에 표시하고, 최종 패키지를 다시 봉인합니다.
