...
HWP 계열, AI 작성 의심, glossary 미정렬, 중복/화수 이상 가능성이 있는 원고는 회차별 수정 전에 전역 컨텍스트 스캔을 먼저 수행한다. 전역 스캔은 수정 지시가 아니라 앞/뒤 모순 분포, 정본 후보, 수정 비용이 낮은 방향을 찾기 위한 지도다.

전역 스캔 전에는 `앞이 틀렸다`, `뒤가 틀렸다`, `실제 역사 기준으로 맞춘다`를 확정하지 않는다. 작중 내부 정합성을 우선하고, 외부 역사 정합화가 뒤쪽 대량 수정을 유발하면 비용 높은 전략으로 분류한다.

## 사전 리스크 체크리스트

- AI 작성/AI-slop 가능성은 증명이 아니라 공격 가설이다. 반복 리캡, 장면 접합, 시간 역류, 정보 상태 회귀, 균질한 문장 리듬을 먼저 의심한다.
- glossary 미정렬 가능성이 높으면 고유명사/실명/가명/약칭/이니셜을 전역 치환하지 않는다. 직접 근거 전에는 정본 보류로 둔다.
- 정합성/중복 가능성이 높으면 중복 회차, 중복 리캡, 사건 상태 회귀, 앞뒤 화 브리지 결손을 root gate로 먼저 본다.
- 인간 검수자 내부 메모 가능성이 있으면 대괄호, 괄호 대안문, 슬래시 병기, 비독자-facing 코멘트를 삭제 전 분류한다.
- HWP 계열에서 화수 표기가 이상하면 회차 번호 누락, 중복 번호, 검수자 삭제로 인한 gap, oversized merged episode를 내용 검수 전 먼저 닫는다.

## Pass 1. 표면 팩트 감리

- `verisimilitude_candidates.jsonl`을 먼저 열어 작중 행동/상태/인과 충돌 후보를 확인한다.
- 날짜, 시간, 금액, 지분율, 나이, 직함, 관계 호칭을 먼저 수집한다.
- 회차별 공백 포함 글자수 `{{minimum_chapter_chars}}`자 미만 후보를 확인한다. 미달 회차는 결락, 중복, 분할 오류, 정본 선택 보류 후보로 본다.
